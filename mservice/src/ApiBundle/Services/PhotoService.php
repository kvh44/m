...
class PhotoService {
    ...
    public function __construct(Registry $doctrine, Translator $translator, UsersService $usersService, CacheService $cacheService, UtileService $utileService, $size_limit_photo, $upload_directory, $profile_photo_directory, $user_photo_directory, $post_photo_directory, $original_directory, $medium_directory, $small_directory, $icon_directory, $medium_photo_max_width, $small_photo_max_width, $icon_photo_max_width, $photo_default_mime_type, $photo_default_type) {
    ...
    protected function uploadFile(Request $request, $is_local = false) {
        ...
        try {
            ...
            $this->utileService->response['success'] = true;
            return $this->utileService->getResponse();
        } catch (\Exception $err) {
            $this->utileService->setResponseState(false);
            $this->utileService->response['success'] = false;
            $this->utileService->setResponseMessage($err->getMessage());
            return $this->utileService->getResponse();
        }
    }
